...
const validTypes = ["ropa", "lencería", "accesorios"];
const validGender = ["men", "women", "kid", "unisex"];
const validSizes = ["XS", "S", "M", "L", "XL", "XXL", "unitalla", "curvy"];
...
const ProductAdminPage: FC<Props> = ({ product }) => {
  ...
  const onDeleted = async (Transition: ComponentType<TransitionProps>) => {
  ...
  const onSubmit = async (form: FormData) => {
    ...
    try {
      ...
      if (!form._id) {
        await router.replace(`/admin/products/${form.slug}`);
      } else {
        setIsSaving(false);
      }
    } catch (error) {
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <Box display="flex" justifyContent="end" sx={{ mb: 1 }}>
          ...
          <Button
            ...
            type="button"
            onClick={() => onDeleted(TransitionLeft)}
            disabled={isSaving}
          >
            Borrar
          </Button>
